const { PANEL_PLANS } = require("./products");
...
function resolvePanelPlan(productId) {
  const plan = PANEL_PLANS[String(productId || "")];
...
function assertPterodactylServerConfig(selectedPlan) {
  resolvePanelPlan(selectedPlan);
...
async function createPterodactylServer({ userId, productId, selectedPlan, username }) {
  const plan = resolvePanelPlan(selectedPlan || productId);
  ...
  const payload = {
    name: `${username}-${selectedPlan || productId}`.slice(0, 191),
    user: Number(userId),
