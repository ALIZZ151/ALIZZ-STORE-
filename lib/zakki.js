...
function getToken() {
  ...
  if (!token) {
    throw new ZakkiError(
      "Token Zakki belum dikonfigurasi.",
      "ZAKKI_TOKEN_MISSING",
      503,
      null,
      getZakkiRuntimeInfo()
...
function getZakkiBodySummary(body) {
  if (!body || typeof body !== "object") {
    return {
      message: safeString(body, 500) || null,
      code: null,
      status: null
    };
  }
...
function logZakkiFailure(event, payload = {}) {
  ...
  console.error(
    ...
    JSON.stringify({
      ...
      responseStatus:
        payload.responseStatus == null ? null : Number(payload.responseStatus),
      ...
      errorMessage: payload.errorMessage
        ? safeString(payload.errorMessage, 700)
        : null,
      responseBody: sanitizeZakkiResponse(payload.body) || null
    })
  );
}
...
function nodeHttpRequest(url, options = {}) {
  return new Promise((resolve, reject) => {
      ...
      (response) => {
        ...
        response.on("data", (chunk) => {
          ...
          if (responseText.length > 1024 * 1024) {
            request.destroy(new Error("Zakki response terlalu besar."));
          }
        });
        ...
        response.on("end", () => {
          resolve({
            ...
            body: parseBodyText(responseText),
    ...
    request.on("timeout", () => {
      request.destroy(new Error("ZAKKI_TIMEOUT"));
    });
...
async function zakkiFetch(path, options = {}) {
  ...
  try {
    const response = await nodeHttpRequest(targetUrl, {
      method,
      body: options.body,
    ...
    if (status < 200 || status >= 300) {
      ...
      if (status === 401 || status === 403) {
        throw new ZakkiError(
          "Token Zakki tidak valid atau tidak punya akses.",
          "ZAKKI_UNAUTHORIZED",
          502,
          data,
          debug
      ...
      if (status === 404) {
        throw new ZakkiError(
          "Transaksi Zakki tidak ditemukan.",
          "ZAKKI_NOT_FOUND",
          404,
          data,
          debug
...
async function createTopup(amount) {
  ...
  if (!Number.isInteger(nominal) || nominal < 1000) {
    throw new ZakkiError(
      "Nominal QRIS minimal Rp1.000.",
      "ZAKKI_INVALID_AMOUNT",
      400,
      null,
      getZakkiRuntimeInfo()
    );
...
async function checkTopup(idTransaksi) {
  ...
  if (!id) {
    throw new ZakkiError(
      "ID transaksi Zakki wajib diisi.",
      "ZAKKI_ID_REQUIRED",
      400,
      null,
      getZakkiRuntimeInfo()
  ...
  return zakkiFetch(`/cektopup?idtopup=${encodeURIComponent(id)}`, {
    method: "GET"
  });
}
...
function normalizeTopupStatus(response) {
  ...
  const kategoriStatus = String(
    response?.kategori_status || data?.kategori_status || ""
  ).toUpperCase();
  ...
  return {
    ...
    id_transaksi:
      ...
      response?.idtopup ||
      null,
    nominal_request: firstNumber(
      rincian.nominal_request,
      data.nominal_request,
      response?.nominal_request
    ),
    kode_unik: firstNumber(
      rincian.kode_unik,
      data.kode_unik,
      response?.kode_unik
    ),
...
async function verifyTopupPaid(idTransaksi, expectedTotal) {
  ...
  if (
    expected > 0 &&
    normalized.total_bayar !== null &&
    Number(normalized.total_bayar) !== expected
  ) {
    return {
      paid: false,
      matched: false,
...
function safeZakkiError(error) {
  ...
  const message =
    code === "ZAKKI_TOKEN_MISSING" || code === "ZAKKI_UNAUTHORIZED"
